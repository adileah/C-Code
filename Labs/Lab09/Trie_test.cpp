...
TEST(remove, 2){
  ...
  a.insert("mary");
  a.insert("greg");
  EXPECT_EQ(a.find("bill"), true);
  a.remove("bill");
  EXPECT_EQ(a.find("bill"), false);
}
...
TEST(words, 1){
  ...
  a.insert("gregory");
  int matches = 0;
  vector<string> ret = a.getAllWords("g");
  for(unsigned i = 0; i < ret.size(); i++){
    string word = ret[i];
    if(word == "greg")matches++;
    if(word == "gregory")matches++;
    if(word == "bill")matches++;
  }
  EXPECT_EQ(matches, 2);
