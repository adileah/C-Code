...
#define LLISTRE_H
/* Name: Adileah Smith
   Partner: Justin Carlton
   Optimizations I did: size, push_back, and at
   Date: Sept 26, 2018
...
class LListRE : public List{
    ...
    public:
      ...
      unsigned size() const{ // Returns the length of the list.
        return my_size; // Recursive version
      }
      ...
      int& at(int loc){// Returns the element at position loc
        ...
        assert(loc >= 0);
        if(last_pos < loc){
          List_Node_t* cur = at(loc - last_pos, last_ptr);
          last_ptr = cur;
          last_pos = loc;
          return cur->data;
        }
        List_Node_t* cur = at(loc, head);
        last_ptr = cur;
        last_pos = loc;
        return cur->data;
      }
      ...
      void remove(int loc){ // Removes the element at position loc
        // if loc is invalid, throw an error.  Allow negative indexes. // Valid index?
        if(loc >= (int)my_size || loc < -(int)my_size){
          throw logic_error("Index not valid");
        }
        if(loc < 0){
  ...
  private:
    ...
    List_Node_t* at(int loc, List_Node_t* cur){
      if(!cur){
        throw logic_error("Index not valid");
      }
      if(loc == 0){
