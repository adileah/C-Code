...
class Trie{
    ...
    vector<unsigned> getAllCount(const string &value)const{
        vector<unsigned> ret;
        bool finder = false;
        Node* found = find(root, value, finder);
        getAllCount(found, ret, value);
        return ret;
    }
    ...
    void getAllCount(Node* node, vector<unsigned> &words, string soFar)const{
        if(!node)return;
        if(node->word)words.push_back(node->count);
        for(unsigned i = 0; i < 26; i++){
            char toAdd = 'a' + i;
            getAllCount(node->children[i], words, soFar + toAdd);
        }
    }
    Node* find(Node* node, const string &word, bool &finder)const{
        if(word == ""){ // Empty string, return if a word ended here
            if(node->word){
                finder = true;
                return node;
        ...
        if(!node->children[ndx])return NULL;  // No node where it should be
        return find(node->children[ndx], word.substr(1), finder);
    }
};
